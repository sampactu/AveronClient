...
public final class AnimationDefinition {
	...
	public int method258(int i) {
		int j = durations[i];
		if (j == 0) {
			Animation class36 = Animation.forId(primaryFrames[i]);
			if (class36 != null)
	...
	private void readValues(Stream stream) {
		...
		while ((i = stream.readUnsignedByte()) != 0) {
			...
			if (i == 1) {
				...
				playerOffhand = stream.readUnsignedShort();
			else if (i == 7)
				playerMainhand = stream.readUnsignedShort();
			else if (i == 8)
				...
				for (int i2 = 0; i2 < len; i2++) {
					otherFrames[i2] = stream.readUnsignedShort();
				}

				for (int i2 = 0; i2 < len; i2++) {
					otherFrames[i2] += stream.readUnsignedShort() << 16;
				}
			} else if(i == 13) {
				int count = stream.readUnsignedByte();
        ...
        while(true) {
            ...
            if (opcode == 0) {
                ...
                for (int i = 0; i < frameCount; i++) {
                    durations[i] = buffer.readShort();
                }
 
                for (int i = 0; i < frameCount; i++) {
                    primaryFrames[i] = buffer.readShort();
                    secondaryFrames[i] = -1;
                }
 
                for (int i = 0; i < frameCount; i++) {
                    primaryFrames[i] += buffer.readShort() << 16;
                }
            } else if (opcode == 2) {
                loopOffset = buffer.readShort();
            } else if (opcode == 3) {
                ...
                playerOffhand = buffer.readShort();
            } else if (opcode == 7) {
                playerMainhand = buffer.readShort();
            } else if (opcode == 8) {
