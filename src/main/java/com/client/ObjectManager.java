...
final class ObjectManager {
			...
			for (int i3 = 0; i3 < 64; i3++) {
				for (int j3 = 0; j3 < 64; j3++)
					if (l2 == i && i3 >= i1 && i3 < i1 + 8 && j3 >= j1 && j3 < j1 + 8)
						method181(l1 + TiledUtils.method156(j3 & 7, j, i3 & 7), 0, stream,
								l + TiledUtils.method155(j, j3 & 7, i3 & 7), k1, j, 0);
					else
			...
			do {
				...
				do {
					...
					if (j3 == i && i3 >= i1 && i3 < i1 + 8 && l2 >= k && l2 < k + 8) {
						...
						int j4 = j + TiledUtils.method157(j1, class46.length, i3 & 7, l2 & 7, class46.width);
						int k4 = k1 + TiledUtils.method158(l2 & 7, class46.length, j1, class46.width, i3 & 7);
	...
	public static void method188(WorldController worldController, int orientation, int y, int type, int plane, CollisionMap collisionMaps,
		...
		if (type == 22) {
			...
			worldController.addGroundDecoration(k1, l2, y, ((Renderable) (obj)), byte1, key, x);
			if (definition.solid && definition.hasActions)
				collisionMaps.method213(y, x);
			return;
		}
		if (type == 10 || type == 11) {
			Object obj1;
			...
			if (definition.solid)
				collisionMaps.method212(definition.impenetrable, definition.width, definition.length, x, y, orientation);
			return;
		}
		if (type >= 12) {
			Object obj2;
		...
		if (type == 0) {
			...
			if (definition.solid)
				collisionMaps.method211(y, orientation, x, type, definition.impenetrable);
			return;
		}
		if (type == 1) {
			Object obj4;
